...
class Auth_model extends Model
{
	...
	function datapost($fields = ''){
    	...
    	$u->{$this->nickcheck} = $this->input->post("nickcheck");
     	...
     	$u->{$this->parent_id} = $this->input->post("parent_id");   	
	...
	function rating($user, $done){
		switch ($done){
			case 'addfriend'	: $add=5; break; //+
			case 'addpredl'		: $add=2; break; //+
			case 'addspros'		: $add=2; break; //+
			case 'contacts'		: $add=5; break;
			case 'contacts2'	: $add=5; break;
			case 'goodusercoment':$add=5; break;
			case 'goodcatcoment': $add=5; break;
			case 'newarticle'	: $add=5; break;
			case 'bue'			: $add=5; break; //+
			case 'sale'			: $add=10;break; //+
			case 'buyinfo'		: $add=10;break;
			
			case 'bedusercoment': $add=-20; break;
			case 'bedcatcoment'	: $add=-10; break;
			//case ''		: $add=0; break;
			
			default: $add=0; break;
		}
		if(!empty($add))
			$this->db->query("UPDATE `".$this->table."` SET `$this->rating` = `$this->rating` + ($add), up = '".date("Y-m-d H:i:s")."' WHERE `".$this->id."`= ".$user);
		//$this->db->update($this->table,array($this->rating => $this->rating))
	}	
	/**
...
function GetUserById($id){
    ...
    {  	
		...
		//$u->ip = getenv(HTTP_X_FORWARDED_FOR);
...
//	        $this->load->model('purses_model');
	        ...
	        {        	
		        ...
		        $this->purses_model->present($result[0]->id, 'reg');
		  		$this->purses_model->present($result[0]->parent_id, 'addfriend');	
		  		$this->rating($result[0]->parent_id, 'addfriend');
	        	return true;
	        }
	        else 
	        	return false;
