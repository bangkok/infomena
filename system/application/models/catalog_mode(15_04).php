...
{ 
  ...
  function getCatalog($lang='no')
  {
	...
	function Cnt($poles){
		...
		if(is_array($poles))
			foreach ($poles as $key => $value)
				$this->db->where(/*$this->table_catalog.'.'.*/$key, $value);
	...
	//COMISIYA	
		...
		$this->db->select("CASE $this->table_catalog.type WHEN 'predl' THEN $this->table_catalog.price_info * $this->comis ELSE price_info END AS cost ");
	//KURS
		...
		$this->db->select("CASE $this->table_catalog.type WHEN 'predl' THEN 
			(price_info / koef * (100- peri)/peri * $this->comis ) ELSE
	...
	//SORT
		...
		$result = $query->result();
		foreach ($result as &$row) $row->price_cash = round($row->price_cash,2);
		return $result;
	}
	function images($pos){
		$pole = 'prevue';
